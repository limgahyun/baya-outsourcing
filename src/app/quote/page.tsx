...
export default function QuotePage() {
  ...
  const handleFunctionCardClick = (functionName: string) => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {FUNCTION_CARDS.map((func) => (
                  <div
                    key={func.id}
                    onClick={() => handleFunctionCardClick(func.name)}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-lg transition-all duration-300
                  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                  hover:border-gray-300"
                placeholder="추가로 필요한 요청사항이 있다면 자유롭게 작성해주세요"
              />
            </div>
          </div>
        </section>
